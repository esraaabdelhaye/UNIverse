...
export class DoctorService {
  ...
  getAllDoctors(page: number = 0, size: number = 10): Observable<ApiResponse<PaginatedResponse<FacultyItem>>> {
    return this.api.get<ApiResponse<PaginatedResponse<FacultyItem>>>(`/doctors?page=${page}&size=${size}`, {withCredentials: true});
  }
  ...
  getDoctorByEmail(email: string): Observable<ApiResponse<FacultyItem>> {
    return this.api.get<ApiResponse<FacultyItem>>(`/doctors/email/${email}`, {withCredentials: true});
  }
  ...
  getDoctorCourses(doctorId: number): Observable<ApiResponse<any[]>> {
    return this.api.get<ApiResponse<any[]>>(`/doctors/${doctorId}/courses`, {withCredentials: true});
  }

  assignCourseToDoctor(doctorId: number, courseId: number): Observable<ApiResponse<string>> {
    return this.api.post<ApiResponse<string>>(`/doctors/${doctorId}/courses/${courseId}`, {}, {withCredentials: true});
  }
}
