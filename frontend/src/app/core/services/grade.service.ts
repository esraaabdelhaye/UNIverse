...
export class GradeService {
  ...
  getStudentGrades(studentId: number): Observable<ApiResponse<Grade[]>> {
    return this.api.get<ApiResponse<Grade[]>>(`/api/grades/student/${studentId}`);
  }

  getCourseGrades(studentId: number, courseId: number): Observable<ApiResponse<Grade[]>> {
    return this.api.get<ApiResponse<Grade[]>>(`/api/grades/student/${studentId}/course/${courseId}`);
  }

  getAverageGrade(studentId: number): Observable<ApiResponse<number>> {
    return this.api.get<ApiResponse<number>>(`/api/grades/student/${studentId}/average`);
  }

  getCourseAverageGrade(studentId: number, courseId: number): Observable<ApiResponse<number>> {
    return this.api.get<ApiResponse<number>>(`/api/grades/student/${studentId}/course/${courseId}/average`);
  }

  getGradesByStatus(studentId: number, status: string): Observable<ApiResponse<Grade[]>> {
    return this.api.get<ApiResponse<Grade[]>>(`/api/grades/student/${studentId}/status/${status}`);
  }
  ...
  updateGrade(submissionId: number, grade: string, feedback?: string): Observable<ApiResponse<Grade>> {
    ...
    return this.api.put<ApiResponse<Grade>>(`/api/grades/submission/${submissionId}${params}`, {});
  }
}
